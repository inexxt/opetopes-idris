...
base_case_k0 {k} p q = FU.unions [FU.fromList (map (\s => F.flip (mul_0k q s)) (MS.toList $ (OU.subopetopes p n))) | n <- natRange (S k)]
...
big_product k curr_faces p q =
    ...
    where
        ...
        build_new_opetopes op = D.trace "build_new_opetopes" $ possible_faces op building_blocks p q where
        ...
        new_faces = F.unions $ map build_new_opetopes (dt "possible_codomains" possible_codomains)
...
mutual
    ...
    small_faces {k1} {k2} p q =
        ...
        FU.unions ([pt_pt, pt_ar, ar_pt, ar_ar] ++ small_products) where
            ...
            small_products =  [fst (product (dt "  r" r) (dt "  s" s)) | n1 <- U.natFromTo Z (S k1),
    ...
    product {k1} {k2} p q = case (p, q) of
            ...
            (Arrow _ _ _, Arrow _ _ _) => D.trace "z" (big_product Z (dtrace $ small_faces p q) p q)

...
instance Eq OpetopeE where
    ...
    (OE (Arrow a1 c1 d1)) == (OE (Arrow a2 c2 d2)) = a1 == a2 && c1 == c2 && d1 == d2
    (OE (Face a c d)) == (OE (Face b c' d')) = a == b -- czemu nie działa && c == c' && d == d'
...
-- Ugly hack for now
oe_to_opetope :: OpetopeE -> SNat n -> Opetope n
oe_to_opetope (OE (Point x)) ZZ = Point x
oe_to_opetope (OE (Arrow s x y)) (SS ZZ) = Arrow s x y
oe_to_opetope (OE (Face s x y)) (SS n) = Face s (oe_to_opetope x n) (oe_to_opetope y n) -- Face s x y
